...
beta_dist_mc <- beta_dist_param(0.30, 0.1^2)
beta_dist_mc
...
model {
    ...
    for (i in 1:N) {
        # Model equation
        M[i] ~ dnorm(pred[i], tau)
        pred[i] <- a_p[i] * f_z[i]^(1 - m_c) * sin_kappa[i]^(m_c) * k_c * k_c1_1[i] * r_tool[i]
    }

    # Priors
    k_c ~ dnorm(0, 0.001)  # Normal prior with mean 0 and precision 0.001
    m_c ~ dunif(0, 1)      # Uniform prior between 0 and 1
