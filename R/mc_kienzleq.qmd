...
```{r}
data_mill_cont <- list("k" = length(dfMcMax$McMax), 
                  "Mc" = Mc,
                  "phi"= phi_real,
                  "ap" = ap, 
                  "fz" = fz,
...
#parameters <- c("mc", "kc1", "mc_estim", "kc1_estim")
parameters_kc11 <- c("kc1", "kc_estim")
```
...
cat("model {
    ...
    for (i in 1:k) {
        # Model equation
        Mc[i] ~ dnorm(pred[i], tau)
        sin_sum_indiv <- 0 # inicial sum
        for (j in 1:z){
            sin_val <-  sin(phi[i] + (j-1)*2*3.141593/z) #
            sin_sum_indiv <- sin_sum_indiv + pow(sin_val, (1-mc))     
        }
        sin_sum[i] <- sin_sum_indiv
...
jags_kienzle_kc11 <- jags(data_mill_cont, inits_kc11, parameters_kc11, model.file="jags_kienzle_cont.txt", 
                      n.iter=5000, n.chains=20, n.burnin=1000)
```
